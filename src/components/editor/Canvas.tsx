...
const EditorCanvas = forwardRef<CanvasHandle>(function EditorCanvas(_, ref) {
  ...
  // ── Gray area marquee selection state ──
      ...
      canvas.on('object:modified', (e) => {
        ...
        if (el && el.type !== 'frame') {
          ...
          if (!alreadyInFrame) {
            ...
            // Auto-unparent: if element was dragged outside its parent frame, release it
            const parentFrame = allEls.find(
              (fe) => fe.type === 'frame' && (fe as FrameElement).childOrder.includes(el.id)
            ) as FrameElement | undefined;
            if (parentFrame) {
              const isOutside = cx < parentFrame.x || cx > parentFrame.x + parentFrame.width ||
                cy < parentFrame.y || cy > parentFrame.y + parentFrame.height;
              if (isOutside) {
                moveOutOfFrame([elementId]);
              }
            }
          }
        }
    ...
    const syncCanvas = async () => {
      ...
      for (const el of sorted) {
        ...
        if (obj) {
          ...
          // Background detection: full-canvas shapes at the bottom of the render
          // stack are made non-interactive so rubber band drag selection works
  ...
  // ── Gray area marquee: drag from outside canvas to select elements ──
  const handleGrayAreaMouseDown = useCallback((e: React.MouseEvent) => {
    if (activeTool !== 'move' || e.button !== 0) return;
    const target = e.target as HTMLElement;
    // If clicking on the Fabric.js canvas, let Fabric handle it
  ...
  useEffect(() => {
    ...
    const handleUp = (e: MouseEvent) => {
      ...
      for (const obj of canvas.getObjects()) {
        ...
        if (oL < selRect.right && oR > selRect.left &&
            oT < selRect.bottom && oB > selRect.top) {
          toSelect.push(obj);
        }
      }
